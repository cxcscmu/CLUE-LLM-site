    ...
    return (
      <div
        ...
        fixed top-16 pr-3 md:w-2/3 w-full r-7 pb-10 ${
          process.env.NODE_ENV === "development" ? "bottom-[calc(12.5rem)]" : "bottom-[calc(10rem)]"
      ...
      >
        {chatLog.map((item) => {
          ...
          const fullThinkMatch = item.content.match(/<think>([\s\S]*?)<\/think>/);
          const openThinkMatch = item.content.match(/<think>([\s\S]*)/);
          const reasoning = fullThinkMatch ? fullThinkMatch[1].trim() : openThinkMatch ? openThinkMatch[1].trim() : "";
                  ...
                  >
                    {reasoning && (
                      <Accordion type="single" collapsible>
                        <AccordionItem value='Reasoning' className="min-w-16 text-black dark:text-zinc-100">
                    ...
                    {messageContent && (
                      <div
                        className="my-2 px-5 py-2 outline-none border border-zinc-100 rounded-xl bg-zinc-100 dark:border-zinc-800 dark:bg-zinc-800 text-black dark:text-zinc-100"
                      >
