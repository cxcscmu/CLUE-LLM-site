...
import { protectedPage, protectedPages } from "@/app/api/password/route";
...
export const Protected: FC<{
  ...
  const [reportText, setReportText] = useState("You have not provided a password.")
  const [nextPage, setNextPage] = useState("/")
  const [nextPageText, setNextPageText] = useState("Please return to the sign-in page.")
  ...
  useEffect(() => {
    const fetchLockedStatus = async () => {
      ...
      if (accessible && redirect) {
        ...
        if (redirect.func) { redirect.func() }
      }
